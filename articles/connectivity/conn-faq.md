...
Networks such as the HSCN and Internet are brought into UKCloud via diverse routes to ensure resilence of the connections. In the case of internet connectivity, it also uses diverse supplier networks to ensure supplier resilience. Once within the UKCloud environment, we use Border Gateway Protocols (BGP) and our private inter-DC fibre netwoks to triangulate the network connections. This means that in the event of an issue with one of networks within a single DC, we can reroute external connectivity via the second DC.
